...
func handleMCPCmd(b backend.Backend, bridge Bridge, args []string, sessionID uuid.UUID, modelID uuid.UUID, agentID uuid.UUID, state *ChatState) (CommandResult, error) {
	for {
...
func doCreateMCPServer(b backend.Backend, bridge Bridge) error {
...
func doUpdateMCPServer(b backend.Backend, bridge Bridge, target models.MCPServerDto) error {
	newName := target.Name
