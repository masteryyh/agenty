...
func handleAgentCmd(b backend.Backend, bridge Bridge, args []string, sessionID uuid.UUID, modelID uuid.UUID, agentID uuid.UUID, state *ChatState) (CommandResult, error) {
	if len(args) > 0 {
...
func switchToAgent(b backend.Backend, bridge Bridge, agentName string) (CommandResult, error) {
...
func doCreateAgent(b backend.Backend, bridge Bridge) error {
...
func doUpdateAgent(b backend.Backend, bridge Bridge, target models.AgentDto) error {
	newName := target.Name
	newSoul := target.Soul
...
func selectAgentModels(b backend.Backend, bridge Bridge, currentIDs []uuid.UUID) ([]uuid.UUID, error) {
