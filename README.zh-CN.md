...
| 指定配置的本地交互模式 | `agenty --config agenty.yaml` | 使用明确指定的配置文件。 |
| Daemon 模式 | `agenty --daemon --config agenty.yaml` | 运行 HTTP 后端服务，供远程客户端连接。 |
...
默认情况下，Agenty 会在当前目录、`./config`、`./configs`、`/etc/agenty` 和 `$HOME/.agenty` 中查找 `agenty.yaml`。你也可以始终通过 `--config` 显式传入配置文件。
...
| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `port` | `8080` | daemon 模式中的 HTTP 监听端口。 |
