 ...
 *
 *   [ bootstrap stub ][ compressed CLI ][ compressed core ][ compressed fileedit ][ footer ]
 *
...
const FILE_EDITOR_TARGET_DIR = join(REPO, "packages/file-editor/target/release");
...
const fileEditorPath = join(FILE_EDITOR_TARGET_DIR, `fileedit${ext}`);
if (!existsSync(fileEditorPath)) {
    throw new Error(`fileedit binary not found at ${fileEditorPath}; run its build first`);
}

const stub = readFileSync(stubPath);
const cli = readFileSync(cliPath);
const core = readFileSync(corePath);
const fileEditor = readFileSync(fileEditorPath);
if (stub.length === 0 || cli.length === 0 || core.length === 0 || fileEditor.length === 0) {
    throw new Error("stub, CLI, core or fileedit binary is empty");
}

const cliSha3 = sha3_256(cli);
const coreSha3 = sha3_256(core);
const fileEditorSha3 = sha3_256(fileEditor);
const cliPayload = await compress(cli);
const corePayload = await compress(core);
const fileEditorPayload = await compress(fileEditor);
...
const footer = encodeFooter(
    ...
    },
    {
        ...
        len: BigInt(fileEditorPayload.length),
        sha3_256: fileEditorSha3,
    },
);

mkdirSync(DIST, { recursive: true });
const out = join(DIST, `agenty-${os}-${arch}${ext}`);
writeFileSync(out, Buffer.concat([stub, cliPayload, corePayload, fileEditorPayload, footer]));
if (os !== "windows") {
    chmodSync(out, 0o755);
}
...
console.log(
    `packed agenty <- ${out}\n` +
        ...
        `  fileedit ${fileEditor.length} -> ${fileEditorPayload.length} bytes (${fileEditorPath})\n` +
        `  total   ${stub.length + cliPayload.length + corePayload.length + fileEditorPayload.length + footer.length} bytes`,
);
