...
pub fn apply_patch(cwd: &Path, patch: &str) -> Result<PatchResult, PatchError> {
    ...
    let read_lock_paths = operation_lock_paths(&groups);
    let lock_directory = lock_directory()?;
    let read_locks = FileLocks::acquire(&lock_directory, &read_lock_paths, LockMode::Shared)?;
...
mod tests {
    ...
    #[test]
    fn reuses_an_existing_lock_file_as_advisory_lock_storage() {
        ...
        fs::write(&lock, b"stale metadata from an older apply_patch\n").unwrap();
    ...
    #[test]
    fn move_creates_locks_for_source_and_destination() {
        ...
        assert!(lock_path(&lock_directory, &source).exists());
        assert!(lock_path(&lock_directory, &destination).exists());
    }
