...
func TestFitCompactedMessagesDropsAssistantContextBeforeUserContext(t *testing.T) {
	t.Parallel()
	...
	fitted := FitCompactedMessages(messages, 100, 0)
	if len(fitted) != 3 {
		t.Fatalf("fitted messages = %+v, want retained user, summary, metadata", fitted)
	}
	if compactionKind(fitted[0]) != "retained_user" {
		t.Fatalf("retained context = %+v", fitted)
	}
}
