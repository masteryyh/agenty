...
func TestModelReasoningEffortIgnoresNonReasoningModel(t *testing.T) {
	model := ModelCallConfig{ModelCode: "gpt-4o", ReasoningEfforts: []shared.ReasoningEffort{}}
...
func TestProviderRequestConversions(t *testing.T) {
	...
	request := ModelCallRequest{
		...
		Messages:        []ModelCallMessage{{Role: conversation.RoleUser, Content: conversation.Text("hello")}},
...
func reasoningModel() ModelCallConfig {
...
func TestProviderToolRegistrationsRejectUnknownToolType(t *testing.T) {
	...
	checks := []func() error{
		func() error {
			_, err := openAIResponsesTools([]ToolDefinition{tool}, false)
			return err
		},
		func() error {
			_, err := openAIChatTools([]ToolDefinition{tool})
			return err
		},
		func() error {
			_, err := anthropicTools([]ToolDefinition{tool})
			return err
		},
		func() error {
			_, err := googleTools([]ToolDefinition{tool})
			return err
		},
	}
...
func TestProviderResponseConversions(t *testing.T) {
	...
	t.Run("OpenAI Responses", func(t *testing.T) {
		...
		assertResponse(t, response, ModelCallStopReasonToolUse, 3, 15)
	...
	t.Run("OpenAI Responses shell call", func(t *testing.T) {
		...
		assertResponse(t, response, ModelCallStopReasonToolUse, 1, 7)
	...
	t.Run("Google GenAI", func(t *testing.T) {
		...
		assertResponse(t, response, ModelCallStopReasonEndTurn, 3, 14)
	})
}
...
func TestSDKMessageConversionsUseTypedVariants(t *testing.T) {
	...
	t.Run("OpenAI Responses", func(t *testing.T) {
		...
		items, err := openAIResponsesMessage(ModelCallMessage{
	...
	t.Run("OpenAI Chat Completions", func(t *testing.T) {
		...
		messages, err := openAIChatMessages(ModelCallMessage{
			Role: conversation.RoleAssistant,
			Content: conversation.Content{
	...
	t.Run("Anthropic Messages", func(t *testing.T) {
		...
		message, err := anthropicMessage(ModelCallMessage{
			Role: conversation.RoleAssistant,
			Content: conversation.Content{
	...
	t.Run("Google GenAI", func(t *testing.T) {
		...
		content, err := googleMessage(ModelCallMessage{
			Role: conversation.RoleAssistant,
			Content: conversation.Content{
				conversation.ToolUseBlock{ID: "call_1", Name: "lookup", Input: shared.RawJSON(`{"q":"x"}`)},
			},
...
func TestShellMessageConversionsAcrossProviders(t *testing.T) {
	...
	responsesCall, err := openAIResponsesMessage(ModelCallMessage{
	...
	compatibleCall, err := openAIResponsesMessage(ModelCallMessage{
		Role: conversation.RoleAssistant, Content: conversation.Content{call},
	}, false)
	...
	chatCall, err := openAIChatMessages(ModelCallMessage{
		Role: conversation.RoleAssistant, Content: conversation.Content{call},
	})
	...
	anthropicCall, err := anthropicMessage(ModelCallMessage{
		Role: conversation.RoleAssistant, Content: conversation.Content{call},
	})
	...
	googleCall, err := googleMessage(ModelCallMessage{
		Role: conversation.RoleAssistant, Content: conversation.Content{call},
	}, nil)
...
func TestApplyPatchMessageConversionsAcrossProviders(t *testing.T) {
	...
	nativeItems, err := openAIResponsesMessages([]ModelCallMessage{assistant, result}, true)
	if err != nil {
		t.Fatal(err)
	}
	...
	freeFormItems, err := openAIResponsesMessages([]ModelCallMessage{assistant, result}, true, true)
	if err != nil {
		t.Fatal(err)
	}
	...
	compatibleItems, err := openAIResponsesMessages([]ModelCallMessage{assistant, result}, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	customItems, err := openAIResponsesMessages([]ModelCallMessage{
	...
	if name := googleToolNames([]ModelCallMessage{assistant})["call_1"]; name != "apply_patch" {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.name == "legacy native" {
				items, err := openAIResponsesMessages([]ModelCallMessage{
...
func TestStreamEventConversions(t *testing.T) {
	...
	tests := []struct {
		name string
		run  func(ModelCallStreamHandler) error
		want ModelCallStreamEvent
	}{
		{
			name: "OpenAI text delta",
			run: func(handler ModelCallStreamHandler) error {
			...
			want: ModelCallStreamEvent{Type: ModelCallStreamEventTextDelta, Index: 2, Delta: "hello"},
		},
		{
			...
			want: ModelCallStreamEvent{Type: ModelCallStreamEventToolInputDelta, Index: 1, Delta: `{"q"`},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var got ModelCallStreamEvent
			if err := tt.run(func(event ModelCallStreamEvent) error {
				got = event
				return nil
			}); err != nil {
...
func TestOpenAIResponsesCustomApplyPatchStreamUsesCompletedFreeformInput(t *testing.T) {
	...
	called := false
	err := emitOpenAIResponsesEvent(func(ModelCallStreamEvent) error {
		called = true
		return nil
	...
	var got ModelCallStreamEvent
	if err := emitOpenAIResponsesEvent(func(event ModelCallStreamEvent) error {
		got = event
		return nil
	}, done); err != nil {
		t.Fatal(err)
	}
	if got.Type != ModelCallStreamEventToolUseDone || got.ToolUseID != "call_1" ||
		!strings.Contains(string(got.ToolInput), `"patch":"*** Begin Patch`) {
		t.Errorf("completed custom apply patch event = %#v", got)
	}
}

func TestCallConfigurationValidation(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := newCaller(context.Background(), tt.model, callConfig{})
...
func TestCallConfigurationControlsOpenAIResponsesToolMode(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			model: ModelCallConfig{
				APIType: APIOpenAI, APIKey: "test-key", ModelCode: "test-model",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			caller, err := newCaller(t.Context(), tt.model, callConfig{})
			if err != nil {
...
func TestNativeOpenAIResponsesProviderRequiresOfficialResponsesAPI(t *testing.T) {
	if !nativeOpenAIResponsesProvider(ModelCallConfig{APIType: APIOpenAI, Official: true}) {
		t.Fatal("official OpenAI Responses provider was not recognized")
	}
	if nativeOpenAIResponsesProvider(ModelCallConfig{APIType: APIOpenAICompletions, Official: true}) {
		t.Fatal("official OpenAI Chat Completions provider was recognized as Responses")
	}
	if nativeOpenAIResponsesProvider(ModelCallConfig{APIType: APIOpenAI}) {
		t.Fatal("compatible Responses provider was recognized as official OpenAI")
	}
}

func testModel() ModelCallConfig {
	return ModelCallConfig{
		APIType:           APIOpenAI,
		APIKey:            "test-key",
		ModelCode:         "test-model",
...
func testShellTool() ToolDefinition {
	return ToolDefinition{
		Type: ToolTypeShell, Name: "shell", Description: "Execute shell commands",
		InputSchema: JSONSchema{
			Type: JSONSchemaTypeObject,
			Properties: map[string]JSONSchema{
				"commands": {
				...
				"timeout_ms":        {Type: JSONSchemaTypeInteger},
				"max_output_length": {Type: JSONSchemaTypeInteger},
			},
...
func testApplyPatchTool() ToolDefinition {
...
func assertResponse(t *testing.T, response *ModelCallResponse, reason ModelCallStopReason, contentLength int, total int64) {
	t.Helper()
