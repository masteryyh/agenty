...
func modelReasoningEffort(model ModelCallConfig, effort shared.ReasoningEffort) (string, error) {
...
func systemPrompt(request ModelCallRequest) (string, error) {
	...
	for index, message := range request.Messages {
		...
		if err != nil {
			return "", fmt.Errorf("modelcall: convert system message %d: %w", index, err)
		}
		if text != "" {
...
func emit(handler ModelCallStreamHandler, event ModelCallStreamEvent) error {
	if handler == nil {
		return nil
	}
	if err := handler(event); err != nil {
		return fmt.Errorf("modelcall: handle stream event %q: %w", event.Type, err)
	}
...
func rawJSON(value any) (shared.RawJSON, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("modelcall: marshal JSON value: %w", err)
	}
