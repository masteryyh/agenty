...
func (properties *JSONSchemaAdditionalProperties) UnmarshalJSON(data []byte) error {
	...
	if len(trimmed) == 0 {
		return fmt.Errorf("modelcall: additionalProperties must not be empty")
	}

	if bytes.Equal(trimmed, []byte("true")) || bytes.Equal(trimmed, []byte("false")) {
		var allowed bool
		if err := json.Unmarshal(trimmed, &allowed); err != nil {
			return fmt.Errorf("modelcall: decode additionalProperties boolean: %w", err)
		}
	...
	if trimmed[0] != '{' {
		return fmt.Errorf("modelcall: additionalProperties must be a boolean or schema object")
	}

	var schema JSONSchema
	if err := json.Unmarshal(trimmed, &schema); err != nil {
		return fmt.Errorf("modelcall: decode additionalProperties schema: %w", err)
	}
...
func (schema JSONSchema) toMap() (map[string]any, error) {
	...
	if schema.ContentSchema != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("modelcall: convert content schema: %w", err)
		}
...
func setJSONSchemaComposition(result map[string]any, schema JSONSchema) error {
	...
	if err != nil {
		return fmt.Errorf("modelcall: convert schema definitions: %w", err)
	}
	setMap(result, "$defs", definitions)

	allOf, err := jsonSchemaList(schema.AllOf)
	if err != nil {
		return fmt.Errorf("modelcall: convert allOf: %w", err)
	}
	setList(result, "allOf", allOf)

	anyOf, err := jsonSchemaList(schema.AnyOf)
	if err != nil {
		return fmt.Errorf("modelcall: convert anyOf: %w", err)
	}
	setList(result, "anyOf", anyOf)

	oneOf, err := jsonSchemaList(schema.OneOf)
	if err != nil {
		return fmt.Errorf("modelcall: convert oneOf: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("modelcall: convert dependent schemas: %w", err)
	}
...
func setJSONSchemaArrays(result map[string]any, schema JSONSchema) error {
	...
	if err != nil {
		return fmt.Errorf("modelcall: convert prefix items: %w", err)
	}
...
func setJSONSchemaObjects(result map[string]any, schema JSONSchema) error {
	...
	if err != nil {
		return fmt.Errorf("modelcall: convert properties: %w", err)
	}
	setMap(result, "properties", properties)

	patternProperties, err := jsonSchemaMap(schema.PatternProperties)
	if err != nil {
		return fmt.Errorf("modelcall: convert pattern properties: %w", err)
	}
	...
	if schema.AdditionalProperties != nil {
		...
		if err != nil {
			return fmt.Errorf("modelcall: convert additionalProperties: %w", err)
		}
...
func setSchema(result map[string]any, key string, schema *JSONSchema) error {
	...
	converted, err := schema.toMap()
	if err != nil {
		return fmt.Errorf("modelcall: convert %s schema: %w", key, err)
	}
