...
func (caller *anthropicCaller) Invoke(ctx context.Context, request ModelCallRequest) (*ModelCallResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("modelcall: invoke anthropic messages API: %w", err)
	}
...
func (caller *anthropicCaller) Stream(
	...
	for stream.Next() {
		...
		if err := accumulator.Accumulate(event); err != nil {
			return nil, fmt.Errorf("modelcall: accumulate anthropic messages stream event: %w", err)
		}
		data, err := json.Marshal(event)
		if err != nil {
			return nil, fmt.Errorf("modelcall: marshal anthropic messages stream event: %w", err)
		}
		var wire anthropicStreamEvent
		if err := json.Unmarshal(data, &wire); err != nil {
			return nil, fmt.Errorf("modelcall: decode anthropic messages stream event: %w", err)
		}
	...
	if err := stream.Err(); err != nil {
		return nil, fmt.Errorf("modelcall: stream anthropic messages API: %w", err)
	}
	...
	for index, block := range final.Content {
		...
		if err := emit(handler, ModelCallStreamEvent{
			Type:      ModelCallStreamEventToolUseDone,
			Index:     index,
			ToolUseID: tool.ID,
			ToolName:  tool.Name,
...
func (caller *anthropicCaller) params(request ModelCallRequest) (anthropic.MessageNewParams, error) {
	...
	for index, message := range request.Messages {
		...
		if err != nil {
			return anthropic.MessageNewParams{}, fmt.Errorf("modelcall: convert anthropic message %d: %w", index, err)
		}
	...
	params := anthropic.MessageNewParams{
		Model:     anthropic.Model(caller.model.ModelCode),
		Messages:  messages,
...
func anthropicToolDefinition(tool ToolDefinition) (anthropic.ToolUnionParam, error) {
	...
	if err != nil {
		return anthropic.ToolUnionParam{}, fmt.Errorf("modelcall: convert Anthropic tool %q schema: %w", tool.Name, err)
	}
...
func anthropicMessage(message ModelCallMessage) (anthropic.MessageParam, error) {
...
func anthropicResponse(result *anthropic.Message) (*ModelCallResponse, error) {
...
func anthropicStopReason(reason string) ModelCallStopReason {
	switch reason {
...
func emitAnthropicEvent(handler ModelCallStreamHandler, event anthropicStreamEvent) error {
	switch event.Type {
	case "content_block_start":
		if event.ContentBlock.Type == "tool_use" {
			return emit(handler, ModelCallStreamEvent{
				Type: ModelCallStreamEventToolUseStart, Index: event.Index,
	...
	case "content_block_delta":
		...
		case "text_delta":
			return emit(handler, ModelCallStreamEvent{Type: ModelCallStreamEventTextDelta, Index: event.Index, Delta: event.Delta.Text})
		case "thinking_delta":
			return emit(handler, ModelCallStreamEvent{Type: ModelCallStreamEventReasoningDelta, Index: event.Index, Delta: event.Delta.Thinking})
		case "input_json_delta":
			return emit(handler, ModelCallStreamEvent{Type: ModelCallStreamEventToolInputDelta, Index: event.Index, Delta: event.Delta.PartialJSON})
		}
	}
