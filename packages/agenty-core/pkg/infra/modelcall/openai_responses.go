...
func (caller *openAIResponsesCaller) Invoke(ctx context.Context, request ModelCallRequest) (*ModelCallResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("modelcall: invoke OpenAI Responses API: %w", err)
	}
...
func (caller *openAIResponsesCaller) Stream(
	...
	var final *ModelCallResponse
	for stream.Next() {
		switch event := stream.Current().AsAny().(type) {
		case responses.ResponseTextDeltaEvent:
			err = emit(handler, ModelCallStreamEvent{
				Type: ModelCallStreamEventTextDelta, Index: int(event.OutputIndex), Delta: event.Delta,
			})
		case responses.ResponseReasoningSummaryTextDeltaEvent:
			err = emit(handler, ModelCallStreamEvent{
				Type: ModelCallStreamEventReasoningDelta, Index: int(event.OutputIndex), Delta: event.Delta,
			})
		...
		case responses.ResponseFunctionCallArgumentsDeltaEvent:
			err = emit(handler, ModelCallStreamEvent{
				Type: ModelCallStreamEventToolInputDelta, Index: int(event.OutputIndex), Delta: event.Delta,
			})
		case responses.ResponseOutputItemAddedEvent:
			switch item := event.Item.AsAny().(type) {
			case responses.ResponseFunctionToolCall:
				err = emit(handler, ModelCallStreamEvent{
					Type: ModelCallStreamEventToolUseStart, Index: int(event.OutputIndex),
		...
		case responses.ResponseOutputItemDoneEvent:
			...
			case responses.ResponseFunctionToolCall:
				err = emit(handler, ModelCallStreamEvent{
					Type: ModelCallStreamEventToolUseDone, Index: int(event.OutputIndex),
					ToolUseID: item.CallID, ToolName: item.Name,
	...
	if err := stream.Err(); err != nil {
		return nil, fmt.Errorf("modelcall: stream OpenAI Responses API: %w", err)
	}
	if final == nil {
		return nil, fmt.Errorf("modelcall: OpenAI Responses stream ended without a completed response")
	}
...
func (caller *openAIResponsesCaller) params(request ModelCallRequest) (responses.ResponseNewParams, error) {
	...
	params := responses.ResponseNewParams{
		Model:           openaishared.ResponsesModel(caller.model.ModelCode),
...
func openAIResponsesMessages(
	...
	for index, message := range messages {
		...
		if err != nil {
			return nil, fmt.Errorf("modelcall: convert OpenAI Responses message %d: %w", index, err)
		}
...
func openAIResponsesCallSources(messages []ModelCallMessage) map[string]openAIResponsesCallSource {
...
func openAIResponsesToolDefinitionWithCodexMode(
	...
	if toolType == ToolTypeShell && nativeOpenAI {
	...
	if err != nil {
		return responses.ToolUnionParam{}, fmt.Errorf("modelcall: convert OpenAI Responses tool %q schema: %w", tool.Name, err)
	}
...
func openAIResponsesResponse(result *responses.Response) (*ModelCallResponse, error) {
	...
	for _, output := range result.Output {
		...
		case responses.ResponseApplyPatchToolCall:
			...
			if err != nil {
				return nil, fmt.Errorf("modelcall: OpenAI Responses returned invalid apply patch operation: %w", err)
			}
		...
		case responses.ResponseCustomToolCall:
			...
			hasToolUse = true
			if item.Input == "" {
				return nil, fmt.Errorf("modelcall: OpenAI Responses returned an empty custom apply patch input")
			}
			content = append(content, conversation.ApplyPatchCallBlock{
...
func emitOpenAIResponsesEvent(handler ModelCallStreamHandler, event openAIResponsesStreamEvent) error {
	switch event.Type {
	case "response.output_text.delta":
		return emit(handler, ModelCallStreamEvent{Type: ModelCallStreamEventTextDelta, Index: event.OutputIndex, Delta: event.Delta})
	case "response.reasoning_summary_text.delta", "response.reasoning_text.delta":
		return emit(handler, ModelCallStreamEvent{Type: ModelCallStreamEventReasoningDelta, Index: event.OutputIndex, Delta: event.Delta})
	case "response.output_item.added":
		if event.Item.Type == "function_call" ||
			...
			return emit(handler, ModelCallStreamEvent{
				Type: ModelCallStreamEventToolUseStart, Index: event.OutputIndex,
				ToolUseID: event.Item.CallID, ToolName: name,
			})
		}
	case "response.function_call_arguments.delta":
		return emit(handler, ModelCallStreamEvent{Type: ModelCallStreamEventToolInputDelta, Index: event.OutputIndex, Delta: event.Delta})
	case "response.output_item.done":
		if event.Item.Type == "function_call" {
			return emit(handler, ModelCallStreamEvent{
				Type: ModelCallStreamEventToolUseDone, Index: event.OutputIndex,
