...
func TestResolveToolApprovalRejectsInvalidInput(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			_, err := resolveToolApproval(hitl.NewManager())(t.Context(), json.RawMessage(tt.params))
...
func TestResolveToolApprovalOverStdio(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		input, err := json.Marshal(map[string]any{
			...
			"params": hitl.Resolution{SessionID: state.Session.ID, RoundID: state.Round.ID, ApprovalID: approvalID, Decision: hitl.Deny},
		})
		if err != nil {
			t.Fatal(err)
		}
		...
		var response struct {
			ID     int             `json:"id"`
			Result hitl.Resolution `json:"result"`
			Error  *rpc.Error      `json:"error"`
		}
		if err := json.Unmarshal(output.Bytes(), &response); err != nil {
			t.Fatal(err)
		}
		if response.Error != nil || response.ID != 1 || response.Result.ApprovalID != approvalID || response.Result.Decision != hitl.Deny {
