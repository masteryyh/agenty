...
func TestEngineHITLLifecycle(t *testing.T) {
	for _, action := range []string{"allow", "deny", "stop", "shutdown"} {
		t.Run(action, func(t *testing.T) {
			...
			resolution := hitl.Resolution{SessionID: session.ID, RoundID: start.RoundID, ApprovalID: request.ApprovalID, Decision: hitl.Allow}
			switch action {
			case "allow", "deny":
				resolution.Decision = hitl.Decision(action)
			...
			if (executed.Load() == 1) != (action == "allow") {
			...
			for _, message := range requests[1].Messages {
				for _, block := range message.Content {
					if toolResult, ok := block.(conversation.ToolResultBlock); ok && toolResult.ToolUseID == "lookup-1" {
						found = true
						if action == "deny" && toolResult.Content[0].(conversation.TextBlock).Text != hitl.DeniedMessage {
