...
| Built-in tools | `t.TempDir()`, helper fixtures, and real filesystem operations | Registration, text-editor reads and writes through `fileedit`, structured `apply_patch` subprocess results, regular-expression search, recursive globbing, directory listing, output limits, and error paths | Yes |
